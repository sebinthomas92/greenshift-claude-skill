...
| `docs/13-marketing-addon.md` | **PREMIUM** - Offer box, comparison table, how-to, review/score box, listing builders, event box |
| `docs/14-animation-addon.md` | **PREMIUM** - Animation container (GSAP), pin scroll, lottie, chained animations, SVG draw |
| `docs/15-query-addon.md` | **PREMIUM** - Query builder, filters, dynamic fields, meta getter, wishlist, visibility conditions |
| `docs/16-woocommerce-addon.md` | **PREMIUM** - WooCommerce query builder, product gallery/tabs/price/rating/hooks, FSE blocks |
...
### Premium Addon Blocks

**Marketing/SEO Addon** (see `docs/13-marketing-addon.md`):
- `greenshift-blocks/offerbox` - Product offer with pricing, coupon, CTA
- `greenshift-blocks/comparison-table` + `comparison-item` - Product comparison
- `greenshift-blocks/howto` + `howto-item` - Step-by-step with HowTo schema
- `greenshift-blocks/reviewbox` - Editor review with criteria scores
- `greenshift-blocks/scorebox` - Product score card with pros/cons
- `greenshift-blocks/event` - Event listing with Event schema
- `greenshift-blocks/offerlisting` - Compact product listing
- `greenshift-blocks/offerlistingfull` - Full listing with expandable content
- `greenshift-blocks/linkhidder` - Hide affiliate links from crawlers

**Animation Addon** (see `docs/14-animation-addon.md`):
- `greenshift-blocks/animation-container` - GSAP animation wrapper
- `greenshift-blocks/pinscroll` - Pin-on-scroll effect
- `greenshift-blocks/lottie` - Lottie/DotLottie files
- `greenshift-blocks/beforeafter` - Before/after image comparison
- `greenshift-blocks/imagesequencer` - Apple-style image sequence
- `greenshift-blocks/videoscroller` - Scroll-driven video playback

**Query Addon** (see `docs/15-query-addon.md`):
- `greenshift-blocks/querygrid` - Advanced post query builder
- `greenshift-blocks/filter` - Dynamic filters for query
- `greenshift-blocks/metagetter` - Retrieve any meta value
- `greenshift-blocks/wishlist` - User wishlist
- `greenshift-blocks/thumbscounter` - Like/vote counter
- `greenshift-blocks/loginpopup` - AJAX login form
- `greenshift-blocks/breadcrumbs` - Breadcrumb navigation
- `greenshift-blocks/visibility` - Conditional block display

**WooCommerce Addon** (see `docs/16-woocommerce-addon.md`):
- `greenshift-blocks/wooquerygrid` - WooCommerce product grid
- `greenshift-blocks/productgallery` - Product image gallery (FSE)
- `greenshift-blocks/producttabs` - Product tabs (FSE)
- `greenshift-blocks/productprice` - Customizable price (FSE)
- `greenshift-blocks/productrating` - Star rating (FSE)
- `greenshift-blocks/productbutton` - Add to cart button (FSE)
- `greenshift-blocks/quickbuy` - Quick buy button (FSE)
- `greenshift-blocks/slidingcart` - Sliding cart panel
- `greenshift-blocks/productswatches` - Color/image swatches
- `greenshift-blocks/stockbar` - Stock progress indicator
- `greenshift-blocks/freeshippingbar` - Free shipping threshold
- `greenshift-blocks/producthooks` - WooCommerce hooks in FSE
- `greenshift-blocks/productdiscount` - Auto-calculated discount badge
...
- `features-grid.html` - Feature cards grid
- `faq-section.html` - FAQ accordion section
- `pricing-table.html` - Pricing comparison
- `cta-banner.html` - Call to action banner
- `footer.html` - Footer layout
- `testimonials-slider.html` - Testimonials carousel
- `stats-counters.html` - Animated counters
- `product-cards-grid.html` - Product card grid

**Premium Addons:**
- `offer-box.html` - Product offer with pricing and coupon
- `comparison-table.html` - Side-by-side product comparison
- `review-box.html` - Editor review with scores
- `score-box.html` - Product score card with pros/cons
- `howto-steps.html` - Step-by-step tutorial with schema
- `listing-builder.html` - Full listing with expandable details
- `animation-container.html` - Scroll-triggered GSAP animation
- `woo-product-grid.html` - WooCommerce product grid
